...
def get_positions(request, account_id):
    ...
    try:
            ...
            if isinstance(positions, list):
                for pos in positions:
                    if isinstance(pos, dict):
                        live_data_redis.publish_position(
                            resolved_hash,
                            {**pos, "updated_at": snapshot_payload["updated_at"]},
                        )
        ...
        return JsonResponse({"error": "Schwab positions API not yet implemented"}, status=501)
