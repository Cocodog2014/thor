...
def _stat_thresholds(symbol: str) -> dict[str, Decimal]:
    ...
    if not required.issubset(out.keys()):
        ...
        logger.warning("SignalStatValue missing for %s: %s (classification disabled for this symbol)", normalized, missing)
        return {}
    return out
...
def _contract_weight(symbol: str) -> Decimal:
    ...
    try:
        ...
        logger.warning("ContractWeight missing for %s (defaulting to 0 => excluded from composite)", normalized)
        return Decimal("0")
...
def classify(symbol: str, net_change: Optional[Decimal | float | int | str]) -> Tuple[Optional[str], Optional[Decimal], Decimal, int]:
    """
