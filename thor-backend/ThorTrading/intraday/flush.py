...
def _update_52w_from_closed_bars(instr_rows: List[InstrumentIntraday]) -> None:
    ...
    for sym, mm in by_symbol.items():
        ...
        if stats.high_52w_date and stats.high_52w_date < (today - timedelta(days=LOOKBACK_DAYS)):
            agg = (
                InstrumentIntraday.objects.filter(symbol=sym, timestamp_minute__gte=window_start).aggregate(h=Max("high_1m"))
            )
        ...
        if stats.low_52w_date and stats.low_52w_date < (today - timedelta(days=LOOKBACK_DAYS)):
            agg = (
                InstrumentIntraday.objects.filter(symbol=sym, timestamp_minute__gte=window_start).aggregate(l=Min("low_1m"))
            )
...
def flush_closed_bars(routing_key: str, batch_size: int = 500, max_batches: int = 20) -> int:
    ...
    for _ in range(max_batches):
        ...
        try:
            ...
            try:
                latest_ts = max([*(r.timestamp_minute for r in instr_rows if r.timestamp_minute)], default=None)
                if latest_ts:
            ...
            logger.exception("Failed bulk insert of %s bars for %s; NACKing batch", len(instr_rows), prefix)
