...
### GET `/api/ThorTrading/quotes/latest`
...
```
thor-frontend/src/pages/ThorTrading/ThorTrading.tsx
```
...
```
http://localhost:8000/admin/ThorTrading/
```
...
```python
from ThorTrading.models import SignalWeight
...
```python
path('api/ThorTrading/', include('ThorTrading.urls')),
```

**Full Endpoint:**
```
http://localhost:8000/api/ThorTrading/quotes/latest
```
...
>>> from ThorTrading.config import TOS_EXCEL_FILE
>>> import os
...
from ThorTrading.models import SignalStatValue, TradingInstrument
...
**Get Latest Quotes:**
```bash
curl http://localhost:8000/api/ThorTrading/quotes/latest
```
...
```javascript
fetch('http://localhost:8000/api/ThorTrading/quotes/latest')
  .then(res => res.json())
  .then(data => {
