        ...
        try:
            ...
            try:
                if self.include_futures and fut_key:
                    result["flushed"]["futures"] = int(
                        flush_closed_bars(fut_key, batch_size=500, max_batches=1) or 0
                    )
                if self.include_equities and eq_key:
                    result["flushed"]["equities"] = int(
                        flush_closed_bars(eq_key, batch_size=500, max_batches=1) or 0
                    )
            except Exception:
