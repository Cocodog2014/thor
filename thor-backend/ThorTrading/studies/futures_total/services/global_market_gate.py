...
from GlobalMarkets.services import get_control_countries, normalize_country_code
...
def _refresh_controlled_countries(force: bool = False) -> Set[str]:
    """Load controlled countries from ThorTrading config with simple TTL caching."""
...
def _heartbeat_mode_active() -> bool:
    """When heartbeat scheduler is active, do NOT start/stop grading services here."""
...
def session_tracking_allowed(market_or_country: Market | str | None) -> bool:
    ...
    country = market_or_country if isinstance(market_or_country, str) else getattr(market_or_country, "country", None)
...
def _is_controlled_market(market: Market | None) -> bool:
    """Return True if market is in the controlled countries set."""
...
def bootstrap_open_markets():
    """On server start, bootstrap workers if markets are already OPEN."""
