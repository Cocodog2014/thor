...
> Consolidated, renumbered, and formatted for clarity. GlobalMarkets is the single timing authority; all session logic and futures capture derive from it.
...
Backend port (dev): [http://127.0.0.1:8000](http://127.0.0.1:8000)
...
Frontend port (dev): [http://127.0.0.1:5173](http://127.0.0.1:5173)
...
## 3. GlobalMarkets Timing & Session Authority

The **World Clock** you see in the UI is powered by the **GlobalMarkets** app on the backend.

At a high level, GlobalMarkets:

* Defines the **9 control markets** (Tokyo, Shanghai, Bombay, Frankfurt, London, Pre_USA, USA, Toronto, Mexico) and their **weights** in a global composite index.
* Tracks each market’s **local trading hours**, **timezone**, and **live status** (`OPEN`, `CLOSED`, `PREOPEN`, `PRECLOSE`, `HOLIDAY_CLOSED`).
* Drives the **global “composite sentiment” score** (0–100) used by the World Clock / Global Markets page.
* Acts as the **single source of truth** for:

  * Which markets are currently trading
  * When they open/close next
  * Whether futures capture / intraday supervisors should be running

The futures side (FutureTrading) listens to GlobalMarkets via:

* The event-driven **MarketMonitor** scheduler, and
* Signals & hooks (`_on_market_open`, `IntradayMarketSupervisor`, etc.)

> **For detailed behavior, APIs, and admin knobs, see `GlobalMarkets.md` in the `GlobalMarkets/` folder.**
> Use that doc any time you need to change how the World Clock / global sessions work.

---

## 4. Data & ML Infrastructure
...
Collector command:
`python manage.py poll_tos_excel` against `RTD_TOS.xlsm` → `LiveData!A1:N13`.
...
## 8. Monitoring & Observability (Concept)
...
## 9. Cloudflare Tunnel (Dev Only – Important)
...
* ✅ **World Clock / GlobalMarkets app** controlling global open/closed state
...
* **BUY/STRONG_BUY**:
  ...
  * `price >= target_high` → `WORKED`
  * `price <= target_low` → `DIDNT_WORK`
* **SELL/STRONG_SELL**:

  * `price <= target_low` → `WORKED`
  * `price >= target_high` → `DIDNT_WORK`
